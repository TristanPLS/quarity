-- =============================================================================
-- Quarity — Schéma OLTP PostgreSQL 16  (Jalon 1 — modélisation métier)
-- =============================================================================
...
--
-- Idempotent : exécutable plusieurs fois (DROP ... CASCADE en tête).
-- =============================================================================
...
-- --- Remise à zéro (ordre inverse des dépendances) ----------------------------
DROP TABLE IF EXISTS audit_log                  CASCADE;
DROP TABLE IF EXISTS exposure_results           CASCADE;
DROP TABLE IF EXISTS tracked_location_profiles  CASCADE;
DROP TABLE IF EXISTS exposure_thresholds        CASCADE;
DROP TABLE IF EXISTS exposure_profiles          CASCADE;
DROP TABLE IF EXISTS notification_deliveries    CASCADE;
DROP TABLE IF EXISTS alert_events               CASCADE;
DROP TABLE IF EXISTS alert_rule_recipients      CASCADE;
DROP TABLE IF EXISTS alert_rules                CASCADE;
DROP TABLE IF EXISTS tracked_location_stations  CASCADE;
DROP TABLE IF EXISTS tracked_locations          CASCADE;
DROP TABLE IF EXISTS aqi_breakpoints            CASCADE;
DROP TABLE IF EXISTS aqi_categories             CASCADE;
DROP TABLE IF EXISTS ref_sensors                CASCADE;
...
CREATE INDEX idx_memberships_user ON memberships (user_id);  -- résolution rapide « mes orgs » (isolation US-09 c4)
...
CREATE UNIQUE INDEX idx_api_tokens_active ON api_tokens (token_hash) WHERE revoked_at IS NULL;  -- lookup auth des clés vives
...
CREATE INDEX idx_ref_locations_country ON ref_locations (country);
...
CREATE INDEX idx_ref_sensors_location ON ref_sensors (ref_location_id);
...
COMMENT ON TABLE aqi_breakpoints IS 'Paliers AQI EPA. unit verrouille l''unité de conc_low/high : doit matcher l''unité OpenAQ canonique du polluant.';
...
CREATE INDEX idx_tracked_locations_org ON tracked_locations (org_id);
...
CREATE TABLE tracked_location_stations (
    id                   BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE INDEX idx_tls_location ON tracked_location_stations (tracked_location_id);
CREATE INDEX idx_tls_station  ON tracked_location_stations (ref_location_id);
-- Au plus 1 station primaire par lieu :
CREATE UNIQUE INDEX uq_tls_primary ON tracked_location_stations (tracked_location_id) WHERE is_primary;
...
CREATE INDEX idx_alert_rules_active ON alert_rules (tracked_location_id, parameter_id) WHERE status = 'active';
...
CREATE UNIQUE INDEX uq_arr_user    ON alert_rule_recipients (alert_rule_id, user_id)     WHERE user_id     IS NOT NULL;
CREATE UNIQUE INDEX uq_arr_email   ON alert_rule_recipients (alert_rule_id, email)       WHERE email       IS NOT NULL;
CREATE UNIQUE INDEX uq_arr_webhook ON alert_rule_recipients (alert_rule_id, webhook_url) WHERE webhook_url IS NOT NULL;
...
CREATE INDEX idx_alert_events_org_fired ON alert_events (org_id, fired_at DESC);
CREATE INDEX idx_alert_events_unread    ON alert_events (org_id, fired_at) WHERE is_read = false;   -- index partiel non-trivial (Jalon 3)
CREATE INDEX idx_alert_events_ranking   ON alert_events (org_id, tracked_location_id, severity, fired_at);  -- ranking US-10
...
CREATE INDEX idx_notif_event  ON notification_deliveries (alert_event_id);
CREATE INDEX idx_notif_status ON notification_deliveries (status) WHERE status IN ('pending','failed','retrying');
...
CREATE TABLE exposure_thresholds (
    id                   BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE tracked_location_profiles (
    id                   BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    tracked_location_id  BIGINT   NOT NULL REFERENCES tracked_locations(id) ON DELETE CASCADE,
...
CREATE INDEX idx_tlp_location ON tracked_location_profiles (tracked_location_id);
